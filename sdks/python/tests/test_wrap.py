"""End-to-end behavior of ``safeloop.init`` + ``safeloop.wrap``.
...
import safeloop
from safeloop._config import get_config
...
@pytest.fixture
def mock_endpoint():
    with respx.mock(base_url="https://stub.safeloop.test") as r:
        yield r
...
def test_wrap_returns_passthrough_for_sync(mock_endpoint):
    ...
    safeloop.init(api_key="sk_live_test", endpoint="https://stub.safeloop.test/v1/traces")

    def agent(msg: str) -> str:
        return f"echo: {msg}"

    wrapped = safeloop.wrap(agent)
...
def test_record_step_appends_to_run(mock_endpoint):
    """safeloop.step(...) inside the wrapped path attaches to the parent run."""
    captured: list = []
    route = mock_endpoint.post("/v1/traces").mock(
        side_effect=lambda req: (
            captured.append(req.read()),
    ...
    def agent():
        safeloop.step(tool_name="classify", kind="llm", input="msg", output="intent", duration_ms=42, status="ok")
        safeloop.step(tool_name="reply", kind="llm", input="intent", output="hello", duration_ms=10, status="ok")
        return "done"
...
def test_record_step_outside_wrap_is_noop():
    """Calling safeloop.step() outside a wrapped agent must not crash."""
    safeloop.init(api_key="sk_live_test")
    # No `wrap`, no run in flight — must be silent.
    safeloop.step(tool_name="orphan", kind="tool", input=None, output=None, duration_ms=1, status="ok")


def test_wrap_with_non_callable_raises():
    safeloop.init(api_key="sk_live_test")
    with pytest.raises(TypeError):
        safeloop.wrap("not callable")  # type: ignore[arg-type]


def test_disabled_flag_skips_ingest(mock_endpoint):
    mock_endpoint.post("/v1/traces").mock(return_value=httpx.Response(200))
    safeloop.init(api_key="sk_live_test", endpoint="https://stub.safeloop.test/v1/traces", enabled=False)

    def agent():
        return "ok"

    assert safeloop.wrap(agent)() == "ok"
    time.sleep(0.2)
    assert not mock_endpoint["/v1/traces"].called


def test_config_singleton_holds_overrides():
    safeloop.init(api_key="sk_live_test", environment="staging", project_name="demo")
    cfg = get_config()
